...
public final class EHudClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        registerKeys();
        HudElementRegistry.addLast(Identifier.fromNamespaceAndPath("ehud", "context_panel"),
                (graphics, tickCounter) -> EHudRenderer.render(graphics));
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            AnimalHudConflictScreen.check(client);
            ClientServerState.tick(client);
            while (openConfig.consumeClick()) {
                VersionClientUi.setScreen(client, new EHudConfigScreen(VersionClientUi.currentScreen(client)));
            }
    ...
    private static void registerKeys() {
        ...
        openConfig = KeyMappingHelper.registerKeyMapping(bindings[0]);
        inspect = KeyMappingHelper.registerKeyMapping(bindings[1]);
        pin = KeyMappingHelper.registerKeyMapping(bindings[2]);
    }
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            if (screen instanceof OptionsScreen) {
                Button button = Button.builder(Component.literal("E HUD Settings"),
                                pressed -> VersionClientUi.setScreen(client, new EHudConfigScreen(screen)))
