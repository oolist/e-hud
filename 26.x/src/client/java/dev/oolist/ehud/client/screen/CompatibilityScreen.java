...
final class CompatibilityScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        Button admin = Button.builder(Component.literal("E HUD Admin"), b -> VersionClientUi.setScreen(minecraft, new AdminScreen(this)))
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        int y = 190;
        graphics.centeredText(font, "Mod Menu: " + installed("modmenu"), width / 2, y, color(installed("modmenu")));
        graphics.centeredText(font, "Animal HUD: " + animalHudStatus(), width / 2, y + 15,
                hasAnimalHud() ? 0xFFFF4D4D : 0xFF42F57B);
        graphics.centeredText(font, "Current server: " + (ClientServerState.available() ? "E HUD available" : "client-side only"),
                width / 2, y + 30, ClientServerState.available() ? 0xFF42F57B : 0xFFFFB13B);
        graphics.centeredText(font, "Remote server features are capability-checked when joining.", width / 2, y + 48, 0xFF9FD8AE);
        graphics.centeredText(font, "Unavailable or operator-disabled controls become locked and clearly labelled.", width / 2, y + 56, 0xFF94A89A);
    }
