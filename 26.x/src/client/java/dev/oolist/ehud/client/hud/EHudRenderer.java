...
import dev.oolist.ehud.client.screen.VersionClientUi;
...
public final class EHudRenderer {
    ...
    public static void renderPreview(GuiGraphicsExtractor graphics, int centerX, int bottomY, EHudConfig config) {
    ...
    private static void renderPanel(GuiGraphicsExtractor graphics, Inspection inspection, EHudConfig config,
                                    boolean preview) {
    ...
    private static void renderPanelAt(GuiGraphicsExtractor graphics, Inspection inspection, EHudConfig config,
        ...
        graphics.text(font, inspection.title(), left + 9, top + 7, config.textColor, true);
        graphics.text(font, inspection.subtitle(), left + 9, top + 18, 0xFF9FD8AE, false);
        ...
        for (int index = 0; index < visibleLines; index++) {
            ...
            if (config.showIcons && config.warningIcons && !icon.isEmpty()
                ...
                graphics.text(font, icon, lineX, lineY, color, true);
                lineX += font.width(icon) + 3;
            }
            graphics.text(font, line.label() + ":", lineX, lineY, 0xFFB7C8BC, config.textShadow);
            graphics.text(font, line.value(), lineX + 4 + font.width(line.label() + ":"), lineY,
    ...
    private static void drawGradientPanel(GuiGraphicsExtractor graphics, int left, int top, int width, int height,
                                          EHudConfig config) {
